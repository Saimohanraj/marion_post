...
class ExampleSpider(scrapy.Spider):
    ...
    async def parse_details_check(self,response,ids):
        item = {}
        ...
        IW_FormClass = "FrmPermitDetail"
        # getting_response_from_back_callback= await self.back_navigation(link,IW_Action,IW_FormName,IW_FormClass,item) 
        # id = re.findall(r'\{\"IW_SessionID_\"\:\s*\"([^>]*?)\"\,',getting_response_from_back_callback.text)[0]
        # track_id=re.findall(r'\"IW_TrackID_\"\:\s*([^>]*?)\}',getting_response_from_back_callback.text)[0]
        # submit = re.findall(r'post\(\"([^>]*?)\"\,',getting_response_from_back_callback.text)[0]
        # back_detail_response= await self.back_click_detail(submit,id,track_id,item) 
    ...
    def parse_details_inside_navigation(self,response,item):
        # item ={}
        item['DBA_Address'] = response.xpath('//span[contains(text(),"Address:")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Qualifier'] = response.xpath('//span[contains(text(),"Qualifier")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Status'] = response.xpath('//span[contains(text(),"Status:")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Class'] = ' '.join(response.xpath('//span[@class="IWDBLABEL4CSS"]/text()|//span[@class="IWDBLABEL12CSS"]/text()').getall())
        item['DBA_State'] = response.xpath('//span[contains(text(),"State #")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Country'] = response.xpath('//span[contains(text(),"County #")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Email'] = response.xpath('//span[contains(text(),"Email:")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Phone'] = response.xpath('//span[contains(text(),"Phone:")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Fax'] = response.xpath('//span[contains(text(),"Fax:")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Expire_Date'] = response.xpath('//span[contains(text(),"Expire Date")]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Expire_Date_2'] = response.xpath('//span[@class="IWLABEL14CSS"]/following-sibling::span[1]/text()').get('').strip()
        item['DBA_Record_Count'] =response.xpath('//span[contains(text(),"Record Count:")]/following-sibling::span[1]/text()').get('').strip()
        empty_list = []
    ...
    async def back_navigation(self, link,IW_Action,IW_FormName,IW_FormClass,item):
        url = f"https://cdplusmobile.marioncountyfl.org{link}$/callback?callback=BTNVIEWPLANREVIEWS.DoOnAsyncClick&x=37&y=20&which=0&modifiers="
        payload={
            IW_Action:"",
        ...
        request = scrapy.FormRequest(url,formdata=payload,headers=self.headers,cb_kwargs={'item':item},dont_filter=True)
        response = await self.crawler.engine.download(request)
        return response
    async def back_click_detail(self, submit,id,track_id,item):
        url=f"https://cdplusmobile.marioncountyfl.org{submit}"
        payload={
            "IW_SessionID_":id,
            "IW_TrackID_":track_id,
